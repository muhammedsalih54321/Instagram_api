...
class Followerprofilescreen extends StatefulWidget {
  final Followersmodel model;
  final int index;
  const Followerprofilescreen(
      {super.key, required this.model, required this.index});

  @override
  State<Followerprofilescreen> createState() => _FollowerprofilescreenState();
}

class _FollowerprofilescreenState extends State<Followerprofilescreen> {
  late Infomodel info;
  ...
  void initState() {
    BlocProvider.of<InfoBloc>(context).add(FetchinfoEvent(
        name: widget.model.data!.items![widget.index].username.toString()));
    BlocProvider.of<HighlightBloc>(context).add(FetchHighlightEvent(
        name: widget.model.data!.items![widget.index].username.toString()));
              ...
              SafeArea(
                child: AppBar(
                  ...
                  leading: IconButton(
                      ...
                      centerTitle: true,
                      title: Text(widget.model.data!.items![widget.index].username.toString(),
                                    ...
                                    return RefreshIndicator(
                                      ...
                                      onRefresh: () async {
                                        return BlocProvider.of<HighlightBloc>(
                                                context)
                                            .add(FetchHighlightEvent(name: widget.model.data!.items![widget.index].username.toString()));
                                      },
                                    );
                                  }
                                        ...
                                        builder: (context, state) {
                                          ...
                                          if (state is Postblocloaded) {
                                            ...
                                            return GridView.builder(
                                              ...
                                              itemBuilder: (context, index) {
                                                print(
                                                    'hi =${post.data!.items![index].thumbnailUrl.toString()}');
                                                return Container(
                                                  child: Image.network(
